...
void sumaNultimos(int a[], int n, int m, int * suma) {
   ...
   int sumaUltimosNumeros = sumaN (subarreglo, m);
   * suma = sumaUltimosNumeros;
}
...
void sumaV2(int a1, int a2, int b1, int b2, Vector *c){
   const int tamanoVectores = 2; 
   Vector* a;
   a = crearVector(tamanoVectores);
   asignarValor(a, 0, a1);
   asignarValor(a, 1, a2);

   Vector* b; 
   b = crearVector(tamanoVectores);
   asignarValor(b, 0, b1);
   asignarValor(b, 1, b2);
